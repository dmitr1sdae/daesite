...
export const isIos11 = () => {
  ...
  return name.toLowerCase() === "ios" && Number.parseInt(version, 10) === 11;
};
...
export const isMinimumSafariVersion = (version: number) =>
  isSafari() && Number(ua.browser.version) >= version;
...
export const doesNotSupportEarlyAccessVersion = () =>
  isSafari() && Number(ua.browser.major) < 14;
