...
import {noop, throttle} from "@daesite/utils";
...
type RateLimiter = <A extends any[]>(
  func: (...args: A) => void,
...
const createObserver = (
  ...
  if (maybeRateLimiter === undefined) {
    maybeRateLimiter = (cb) =>
      throttle(cb, 16, {leading: true, trailing: true});
  }
  ...
  const handleResize = maybeRateLimiter
    ? maybeRateLimiter(handleResizeCallback)
